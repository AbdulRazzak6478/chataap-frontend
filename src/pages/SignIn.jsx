...
const SignUp = () => {
  ...
  async function onSubmitData(event) {
    ...
    console.log(
      `userid : ${user.data.data.userData.id} storing in localstorage`
    );

    // storing user id into local storage
    localStorage.setItem(
      "chatAppUserId",
      JSON.stringify(user.data.data.userData.id)
    );
  ...
  useEffect(() => {
    ...
    console.log("last 365 days date : ", startDate);
    (async () => {
      const dashboardApi = await axios.get(
        `http://localhost:4000/api/v2/admin/dashboard/YVS768QX30CCF28LUF?time=${startDate}`
      );
      console.log(" calling dashboard api  : ", dashboardApi);
    })();
  }, []);
    // const [participants, setParticipants] = useState([]);
    const fun  = async()=>{
      console.log('before')
      const response = await fetch(`http://localhost:3005/api/tournaments/getRoundDataForFixing/6696286a2ad81cc25b1e8243`, {
        method: 'GET',
        headers: {
  ...
  return (
    <>
      {token ? (
        <Navigate
          to={`/${JSON.parse(localStorage.getItem("chatAppUserId"))}`}
          replace={true}
        />
      ) : isSignIn ? (
        <Navigate
        ...
        <div className="signup">
          ...
          <form action="">
            ...
            <button
              type="submit"
              className={`register sign ${loading ? "load" : ""}`}
